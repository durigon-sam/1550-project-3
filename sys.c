...
//This is where My work will be implemented
...
void enqueue(struct cs1550_sem* sem){

	//create a new node and set it to the current task
	struct semNode* newNode = (struct semNode*)kmalloc(sizeof(struct semNode), GFP_KERNEL);
	newNode->next = NULL;
	//saving the current process's task_struct
	newNode->task = current;

	//if queue is empty, add this to the beginning of the queue
...
struct semNode* dequeue(struct cs1550_sem* sem){

	//preserve the node to be removed for return
	struct semNode* node = sem->head;

	//if the queue is not empty, change the head
	if (node != NULL){
		//if there is only one thing in the queue, empty the queue
...
SYSCALL_DEFINE1(cs1550_down, struct cs1550_sem*, sem){
	mutex_lock(&sem_lock);
...
SYSCALL_DEFINE1(cs1550_up, struct cs1550_sem*, sem){
	struct semNode* node;
	mutex_lock(&sem_lock);
	//value += 1
	sem->value += 1;
...
/*
SYSCALL_DEFINE1(cs1550_up, struct cs1550_sem *sem){
    return sys_cs1550_up(sem);
}

SYSCALL_DEFINE1(cs1550_down, struct cs1550_sem *sem){
    return sys_cs1550_down(sem);
}
*/
/*
